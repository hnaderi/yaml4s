...
object Main {
  def main(args: Array[String]): Unit = Zone { implicit zone =>
    val input =
      scala.io.Source.fromFile("yaml.org.yaml").getLines().mkString("\n")
    ...
    visitNode(document, root, Printer)
  ...
  def visitNode(
    ...
    nodeType match {
      case yaml_node_type_e.YAML_MAPPING_NODE =>
        visitMapping(document, node.data.mapping, visitor)
      case yaml_node_type_e.YAML_SEQUENCE_NODE =>
        visitSeq(document, node.data.sequence, visitor)
      case yaml_node_type_e.YAML_SCALAR_NODE =>
        visitScalar(node.data.scalar, visitor)
  ...
  def visitScalar(
    ...
    visitor.onScalar(scalarValue(node))
  ...
  def visitSeq(
    ...
    while (cur.toLong < items.top.toLong) {
      ...
      visitNode(document, node, visitor)
      cur = cur + 1
    }
  ...
  def visitMapping(
    ...
    while (cur.toLong < items.top.toLong) {
      ...
      visitor.goto(scalarValue((!key).data.scalar))
